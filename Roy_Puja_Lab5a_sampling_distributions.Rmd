...
ggplot(samp1, aes(x = scientist_work)) +
  ...
  labs(
    x = "", y = "",
    title = "A smaller population sample of the Question:"
  ) +
  coord_flip() 
```
...
**The distribution of responses in this sample is rather skewed toward people who believe that the work scientists does benefits the people. Whereas, when you compare to the larger data of global_monitor, it seems that he sample is pointing to the entire population.**
...
**I don’t expect the sample proportion to match the sample proportion of another student's sample proportion because the sample is random and there could be a case in  where a student can have extensive “benefits” in their table compared to other students.**
...
**According to the histogram, I observe about 2000 elements in the graph. The sampling distribution looks symmetric with no skew and there is a prominent center at 0.2 or 20%.**
...
**There are 24 observations in sample_props_small, each observation represents a proportion of response in each sample that believes that scientists doesn’t benefit them.**
...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      verbatimTextOutput("sampling_results")
    )
  )
)
...
server <- function(input, output) {
  ...
  output$sampling_results <- renderPrint({
    cat("Samples of size 10 of 5000 simulations:\n")
    print(sample_props_small1)
    cat("\nSamples of size 50 of 5000 simulations:\n")
    print(sample_props_small2)
    cat("\nSamples of size 100 of 5000 simulations:\n")
    print(sample_props_small3)
  })
}
...
**Based on my sample, over 93% of people think that the work scientists does benefits them daily. My best point estimate of the population proportion would be around 80% to 85%**
...
**According to the average of p_hat, approximately 80% of the population believe that scientists do enhance their daily lives.**
...
**The data looks symmetrical and there are no skews like the graph in exercise 8. The calculated mean to find proportion population shows results of around 80% again.**
...
**Observing the charts, I would measure and state that chart 2 has a smaller spread because it would be useful to work with smaller samples. Those smaller samples generally would provide a precisely accurate representation of the population proportion.**
