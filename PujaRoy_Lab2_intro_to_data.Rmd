...
**The first and second histogram show more information or more bins than the third histogram which displays 3 bins. The second histogram displayed accurate details since there were 13 bins to present trends of flight delays. The third histogram was the most obscure since it portrayed 3 bins to support 32,735 observations of 16 variables.**
...
```{r}
sfo_feb_flights <- nycflights %>%
  filter(dest == "SFO", month == 2) # Filter destination column in short for dest and indicate which month
...
**Shown above is the distribution of the arrival delays using a histogram with 11 bins to present information.**
...
**Delta (DL) and United Airlines (UA) have the largest IQR values (Q3 - Q1) and similarly the most variable arrival delays.**
...
**The pros are that the lowest mean would provide more insights regarding average departure delay whereas the lowest median would not provide more trends in outliers and provide information of the middle of the dataset. In this case, the lowest mean would be the best option of identifying accurate results on departure delays. In this case, July (month 7) is the month with the highest average departure delay.**
...
**I would select Laguardia (LGA) airport since it departs on time and they have less delayed departures compared to EWR and JFK airports.**
...
ggplot(data = nycflights, mapping = aes(x = distance, y = avg_speed)) +
        geom_point()
```
**The relationship displays that the average speed increases until the distance is in the range of 1000-1500 (miles) and then it becomes stable.**
...
**Based on plot, the cutoff point is approximately 20mins. After this point, as departure delays increase, similarly arrival delays increase.**
